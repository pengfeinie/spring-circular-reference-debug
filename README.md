...
![image-20191221122422289](./images/test/image-20191221122422289.png)
...
![image-20191221122804919](./images/test/image-20191221122804919.png)
...
![image-20191221122955274](./images/test/image-20191221122955274.png)
...
![image-20191221123453757](./images/test/image-20191221123453757.png)
...
![image-20191221124206790](./images/test/image-20191221124206790.png)
...
![image-20191221124728063](./images/test/image-20191221124728063.png)

步骤七：这行代码返回的肯定是false，也就意味着这里代码直接返回了。

![image-20191221124914927](./images/test/image-20191221124914927.png)

返回出去的singletonObject肯定是 null .

![image-20191221125116205](./images/test/image-20191221125116205.png)

步骤八：到这里，getSingleton(beanName)这个方法已经在这个流程中被我们分析完了。

![image-20191221125234569](./images/test/image-20191221125234569.png)

步骤九：既然我们从这个singletonObjects单例缓存池（也就是一级缓存）拿不到我们想要的bean的实例，也就说明我们想要的bean其实并没有被实例化。那么怎么办呢？ 我们只能去实例化我们想要的bean的实例了。

![image-20191221125408433](./images/test/image-20191221125408433.png)

你会发现，在这里调用了getSingleton(String beanName, ObjectFactory<?> singletonFactory)方法，源码中是lamad表达式。这里是非常关键的。

![image-20191221125602636](./images/test/image-20191221125602636.png)
...
![image-20191221130806840](./images/test/image-20191221130806840.png)

当执行到这里的时候，就是要创建我们的bean了。我们可以看到，singletonFactory是刚才我们传进来的那个lamad表达式，当调用这个lamad表达式的getObject()方法的时候。

![image-20191221145550657](./images/test/image-20191221145550657.png)

实际上getObject()方法内部是会去执行createBean(beanName, mbd, args)方法的。

![image-20191221145856447](./images/test/image-20191221145856447.png)

我们接下去继续看createBean(beanName, mbd, args)方法。当我们进去看的时候，发现里面调用了

doCreateBean(beanName, mbdToUse, args);

![image-20191221150100698](./images/test/image-20191221150100698.png)

我们接着进去看doCreateBean(beanName, mbdToUse, args)方法。

![image-20191221150218421](./images/test/image-20191221150218421.png)
...
![image-20191221150358071](./images/test/image-20191221150358071.png)

通过观察，我们发现roleService这个对象已经被我们new出来了，只是里面的userService这个属性是null，还没有被我们赋值而已。

![image-20191221150651357](./images/test/image-20191221150651357.png)

成功new出roleService这个对象之后，这个方法被返回，我们再次回到doCreateBean方法内部。如下：

![image-20191221151124248](./images/test/image-20191221151124248.png)
...
![image-20191221151535002](./images/test/image-20191221151535002.png)
...
![image-20191221151912774](./images/test/image-20191221151912774.png)

接下去，我们需要处理roleService这个对象中的属性的注入了，就是在这个方法中处理的。

![image-20191221152054784](./images/test/image-20191221152054784.png)

我们进入到这个方法的内部。

![image-20191221152718201](./images/test/image-20191221152718201.png)

解析过程如下：

![image-20191221153922402](./images/test/image-20191221153922402.png)

我们再进入到metadata.inject(bean, beanName, pvs)方法内部继续查看。

![image-20191221154109664](./images/test/image-20191221154109664.png)

我们继续进入查看。

![image-20191221154239007](./images/test/image-20191221154239007.png)

到这里，似乎快看到希望了，继续进去查看。

![image-20191221154418069](./images/test/image-20191221154418069.png)

再进去查看，如下图所示：

![image-20191221154614113](./images/test/image-20191221154614113.png)
...
![image-20191221154948786](./images/test/image-20191221154948786.png)

我们再继续往下看的时候，在这里已经很明显了。我们就是那userService这个bean的实例了。

![image-20191221155411899](./images/test/image-20191221155411899.png)

我们进入到descriptor.resolveCandidate(autowiredBeanName, type, this)方法内部去看一下。

![image-20191221155545398](./images/test/image-20191221155545398.png)

天啊，这段代码太熟悉了，getBean(String beanName), 接下来就和我们之前分析的套路是一样的。

![image-20191221155834230](./images/test/image-20191221155834230.png)
...
![image-20191221155953658](./images/test/image-20191221155953658.png)
...
![image-20191221160533032](./images/test/image-20191221160533032.png)

我们知道，当前的bean的实例是userService，现在需要去依赖roleService。我们再进去看。

![image-20191221160640514](./images/test/image-20191221160640514.png)

现在我们需要去拿roleService，这段代码太熟悉了。

![image-20191221160940130](./images/test/image-20191221160940130.png)

我们又回到了这里，不过这次的处理就有点不一样了，我们进去看一下。

![image-20191221161044022](./images/test/image-20191221161044022.png)
...
![image-20191221161501810](./images/test/image-20191221161501810.png)

从singletonObjects这个一级缓存中没有拿到，继而再从earlySingletonObjects这个二级缓存中去拿，还是没有拿到，所以就从singletonFactories这个三级缓存中去拿了。

![image-20191221161648153](./images/test/image-20191221161648153.png)
...
![image-20191221161945226](./images/test/image-20191221161945226.png)
...
![image-20191221162431237](./images/test/image-20191221162431237.png)

请注意，到这个地方的时候，userService的属性还是没有被设置进去的，不过执行完上面两行代码的时候，属性就被设置进去了。

![image-20191221162609050](./images/test/image-20191221162609050.png)
...
![image-20191221162900906](./images/test/image-20191221162900906.png)

现在实例化结束了，我们需要做一些收尾工作了。

![image-20191221163010199](./images/test/image-20191221163010199.png)
...
![image-20191221163120982](./images/test/image-20191221163120982.png)

接下来就是执行addSingleton(beanName, singletonObject)方法，很明显，就是将实例化好的对象放入到singletonObjects这个一级缓存中去。

![image-20191221163239943](./images/test/image-20191221163239943.png)

我们进去看一下，它将userService这个bean对象放入到singletonObjects一级缓存，并从二级缓存earlySingletonObjects和三级缓存singletonFactories中移除这个bean。

![image-20191221163355134](./images/test/image-20191221163355134.png)

真的快搞完了，，这么久了。现在UserService彻底结束。

请注意，现在roleService里面的属性还是null的。

![image-20191221163720261](./images/test/image-20191221163720261.png)

不过当我们执行上面的两行代码。属性值就被设置进去了。

![image-20191221163805417](./images/test/image-20191221163805417.png)
